...
-- 부서
DROP TABLE IF EXISTS lms_dpt RESTRICT;
...
CREATE TABLE lms_stud (
  ...
  bann0 INTEGER     NULL     COMMENT '은행번호' -- 은행번호
)
COMMENT '학생';
...
CREATE TABLE lms_room (
  ...
  cenno VARCHAR(50) NOT NULL COMMENT '교실명', -- 교실명
  title INTEGER     NOT NULL COMMENT '수용인원', -- 수용인원
  capa  INTEGER     NOT NULL COMMENT '지점번호' -- 지점번호
)
COMMENT '교실';
...
CREATE TABLE lms_lect (
  ...
  hrs     INTEGER      NULL     COMMENT '총강의시간' -- 총강의시간
)
COMMENT '교육과정';
...
-- 메니저
ALTER TABLE lms_mgr
  ADD CONSTRAINT PK_lms_mgr -- 메니저 기본키
    PRIMARY KEY (
      mno -- 매니저번호
    );
...
CREATE TABLE lms_center (
  canno INTEGER     NOT NULL COMMENT '지점번호', -- 지점번호
...
ALTER TABLE lms_center
  MODIFY COLUMN canno INTEGER NOT NULL AUTO_INCREMENT COMMENT '지점번호';

-- 은행
CREATE TABLE lms_bank (
  bann0 INTEGER     NOT NULL COMMENT '은행번호', -- 은행번호
...
ALTER TABLE lms_bank
  MODIFY COLUMN bann0 INTEGER NOT NULL AUTO_INCREMENT COMMENT '은행번호';
...
-- 부서
ALTER TABLE lms_dpt
  ADD CONSTRAINT PK_lms_dpt -- 부서 기본키
    PRIMARY KEY (
      dno -- 부서번호
    );
...
-- 교육과정
ALTER TABLE lms_lect
  ADD CONSTRAINT FK_lms_mgr_TO_lms_lect -- 메니저 -> 교육과정
    FOREIGN KEY (
      mno -- 매니저번호
    )
    REFERENCES lms_mgr ( -- 메니저
      mno -- 매니저번호
    );
...
-- 메니저
ALTER TABLE lms_mgr
  ADD CONSTRAINT FK_lms_user_TO_lms_mgr -- 사용자 -> 메니저
    FOREIGN KEY (
      mno -- 매니저번호
    )
    REFERENCES lms_user ( -- 사용자
...
-- 메니저
ALTER TABLE lms_mgr
  ADD CONSTRAINT FK_lms_posi_TO_lms_mgr -- 직급 -> 메니저
    FOREIGN KEY (
      pno -- 직급번호
    )
...
-- 메니저
ALTER TABLE lms_mgr
  ADD CONSTRAINT FK_lms_dpt_TO_lms_mgr -- 부서 -> 메니저
    FOREIGN KEY (
      dno -- 부서번호
    )
    REFERENCES lms_dpt ( -- 부서
      dno -- 부서번호
    );
...
-- 부서
ALTER TABLE lms_dpt
  ADD CONSTRAINT FK_lms_center_TO_lms_dpt -- 지점 -> 부서
    FOREIGN KEY (
      canno -- 지점번호
    )
    REFERENCES lms_center ( -- 지점
